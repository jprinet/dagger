...
public final class AggregatedDepsProcessor extends BaseProcessor {
      ...
      if (!MoreElements.getPackage(module).getQualifiedName().toString().startsWith("dagger.hilt")) {
        ...
        ImmutableList<TypeElement> hiltInternalModules =
            replacedModules.stream()
                .filter(
                    replacedModule ->
                        MoreElements.getPackage(replacedModule)
                            .getQualifiedName()
                            .toString()
  ...
  private static Optional<ClassName> getAnnotation(
    ...
    return Optional.of(Iterables.getOnlyElement(usedAnnotations));
  }
  ...
  private static boolean isDaggerGeneratedModule(Element element) {
    ...
    return element.getAnnotationMirrors().stream()
        ...
        .map(mirror -> asString(Iterables.getOnlyElement(asList(AnnotationMirrors.getAnnotationValue(mirror, "value")))))
